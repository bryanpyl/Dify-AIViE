...
const Paragraph = ({paragraph, nodeTitle, botResponse}:{paragraph:any, nodeTitle?:string, botResponse?:boolean}) => {
  ...
  return (
  <>
    <p data-response={botResponse?nodeTitle:'user-query'}>{paragraph.children}</p>
  </>)
}
...
export function Markdown(props: { content: string; className?: string, nodeResponse?:string, aivieApp?:string, botResponse?:boolean}) {
  const latexContent = preprocessLaTeX(props.content)
  const botResponse = props.botResponse
  // const responseType = props.nodeResponse?.includes('Hold Response')?'hold-response':'final-response'
  const responseType = useMemo(()=>{
    if (props.nodeResponse?.includes('Hold Response')){
  ...
  useEffect(()=>{
    ...
    if (!container) return
    if (responseType==='hold-response'){
      setChatThinking(true)
    }
    if (responseType==='final-response'){
      setChatThinking(false)
    }
  },[responseType])
  return (
    <div ref={containerRef} className={cn(props.className, 'markdown-body')}>
      <ReactMarkdown
        ...
        components={{
          ...
          audio: AudioBlock,
          //   a: Link,
          //   p: (props) => <Paragraph paragraph={props} botResponse={botResponse} nodeTitle={responseType} />,
          //   // p: Paragraph,
      ...
      {chatThinking?
      <ChatThinking app_agent={props.aivieApp!}/>
      :null
      }
    </div>
  )
