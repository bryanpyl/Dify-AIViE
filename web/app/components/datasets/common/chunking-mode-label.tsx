  ...
  return (
    <Badge>
      <div className='flex h-full items-center space-x-0.5 text-text-tertiary'>
        ...
        <span className='system-2xs-medium-uppercase'>{isGeneralMode ? `${t('datasetDocuments.list.chunkingMode.general')}${generalSuffix}` : t('datasetDocuments.list.chunkingMode.parentChild')}</span>
      </div>
    </Badge>
  )
}
